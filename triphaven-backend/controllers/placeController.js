...
const addDestination = async (req, res) => {
  try {
    ...
    const checkDestinationExist = await Destination.findOne({
      destinationName,
    });
    if (checkDestinationExist)
      return res
        ...
        .json({
          success: false,
          message: "This destination already in the system.",
        });
...
const getDestinationList = async (req, res) => {
  try {
    const destinationList = await Destination.find().populate(
      "town",
      "townName"
    );
...
const getSingleDestinationById = async (req, res) => {
  try {
    ...
    const destinationDetails = await Destination.findById(id).populate(
      "town",
      "townName"
    );
